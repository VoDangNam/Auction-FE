<template>
  <!-- Section 1 -->
  <section class="section1 container w-100 mb-5">
    <div class="container">
      <div class="row">
        <div class="col-lg-7 col-md-12 col-sm-12 d-flex flex-column justify-content-center">
          <div class="d-flex flex-column gap-5">
            <div class="">
              <h3 class="mb-4 fs-4 fw-bold d-flex flex-column justify-content-start text-success">
                Discover Extraordinary Art at Auction
              </h3>
              <p class="mb-4 ">
                Discover a world of exquisite art pieces and unique collectibles at ArtAuction.
                Whether you're an avid collector or a first-time buyer, our platform offers a seamless
                and exciting auction experience.
              </p>
            </div>
            <div class="button d-flex gap-4">
              <button class="btn btn-success w-25 text-nowrap" type="button">Explore Now</button>
              <button class="btn btn-outline-success w-25 text-nowrap" type="button">See details</button>
            </div>
          </div>
        </div>

        <div class="col-lg-5 col-md-12 col-sm-12 d-flex justify-content-lg-end justify-content-md-center mt-md-4 ">
          <div class=" position-relative d-flex justify-content-end">
            <img src="/src/assets/img/1.png" alt="Art Auction" class="img-fluid w-50 h-75 my-5 d-none d-lg-inline" />
            <img src="/src/assets/img/2.png" alt="Art Auction" class="img-fluid  h-50 me-5 ms-3 d-none d-lg-inline" />
            <img src="/src/assets/img/3.png" alt="Art Auction"
              class="img-fluid position-absolute w-25 h-50 bottom-0 pt-4 d-none d-lg-inline" />
          </div>

        </div>
      </div>
      <!-- <div class="picture col-lg-6 col-md-12 col-sm-12 container">
          <div class="big-picture">
            <img src="/src/assets/img/1.png" alt="Art Auction" class="img-fluid" style="width: 250px; height: auto" />
          </div>
          <div class="small-picture" style="display: flex; flex-direction: column; gap: 20px">
            <img src="/src/assets/img/2.png" alt="Art Auction" class="img-fluid"
              style="width: 180px; height: auto; display: flex; justify-self: flex-start" />
            <img src="/src/assets/img/3.png" alt="Art Auction" class="img-fluid"
              style="width: 180px; height: auto; display: flex; margin-left: 100px" />
          </div>
        </div> -->

    </div>
  </section>

  <!-- Section 2 -->
  <section class="section2 row mb-5">
    <div class="col-lg-12 col-md-12 col-sm-12 d-flex justify-content-center align-items-center text-white">
      <div class="text-center container my-5">
        <h3 class="fw-bold ">Where Art Meets Passion</h3>
        <p class="m-0">
          Explore a colorful treasure trove of creative work and open up investment opportunities in
          one-of-a-kind masterpieces.
        </p>
      </div>
    </div>

  </section>

  <!-- Section 3 -->
  <div class="container">
    <section class="section3 ">
      <div class="row d-flex align-items-center">
        <div class="col-lg-7 col-md-6 col-sm-12 h-100">
          <div class="row">
            <div class="col-lg-6 d-flex ">
              <img src="/src/assets/img/4.png" alt="Art Auction"
                class="img-fluid img border border-3 border-success rounded-4 p-3 w-100 h-lg-75 h-sm-100 my-lg-auto  m-md-0" />
            </div>
            <div class="col-lg-5 d-flex justify-content-between flex-column">
              <img src="../../../assets/img/hoasen.png"
                class="w-75 border border-3 border-success rounded-4 p-2 ms-5 d-none d-lg-inline" alt="">
              <img src="../../../assets/img/hoasen.png"
                class="w-50 border border-3 border-success rounded-4 p-2 mt-4 me-3 d-none d-lg-inline" alt="">
            </div>
          </div>

        </div>

        <div class="col-lg-5 col-md-6 col-sm-12 p-lg-0 ps-md-5 mt-5 m-md-0">
          <h3 class="fw-bold text-success">Discover & Own Great Works of Art</h3>
          <p class="">Join the global community of collectors in the journey of auctioning unique, museum-worthy
            masterpieces</p>
          <p class="">
            Open up the opportunity to own and invest in timeless masterpieces.
          </p>
          <button class="btn btn-success w-50 mt-5">See details</button>

        </div>
      </div>

      <div class="row mt-5">
        <div class="col-lg-12 col-md-12 col-sm-12 text-center">
          <h3 class="fw-bold text-success">Bringing Art to You</h3>
          <p class="m-0">Participate in online auctions, transparent and secure.</p>
        </div>
      </div>

      <div class="row d-flex">
        <div class="col-lg-4 col-md-4 col-sm-12 d-flex  mt-4">
          <div class="card w-100">
            <div class="card-body d-flex align-items-center flex-column">
              <img src="/src/assets/img/Law.png" class="p-4 w-100px h-100px" alt="..." />
              <div class="text-center">
                <h5 class="fw-bold text-success">Easy Participation</h5>
                <p class="card-text">Quick registration, join the auction with just a few steps.</p>
              </div>
            </div>

          </div>
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 d-flex  mt-4">
          <div class="card w-100">
            <div class="card-body d-flex align-items-center flex-column">
              <img src="/src/assets/img/Lock.png" class="p-4 w-100px h-100px" alt="..." />
              <div class="text-center">
                <h5 class=" fw-bold text-success">Transparent and Safe</h5>
                <p class="card-text">
                  The entire auction process is guaranteed to be clear, fair and confidential.
                </p>
              </div>
            </div>

          </div>
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 d-flex  mt-4">
          <div class="card w-100">
            <div class="card-body d-flex align-items-center flex-column">
              <img src="/src/assets/img/palette-solid-full.svg" class="p-4 w-100px h-100px" alt="..." />
              <div class="text-center">
                <h5 class="fw-bold text-success">Unique Masterpiece</h5>
                <p class="card-text">Opportunity to own works of art with timeless value.</p>
              </div>
            </div>

          </div>
        </div>
      </div>
    </section>
  </div>


  <!-- Section 4 -->
  <section class="section4 row bg-success overflow-hidden mt-5">

    <div class="col-lg-6  position-relative ">
      <img src="/src/assets/img/flower.png" alt="Flower" class="me-5 d-none d-lg-inline" />
    </div>
    <div class="col-lg-6 col-md-12 col-sm-12 ">
      <div class="my-5 text-white py-3 mx-5">
        <h3 class="fw-bold">Breath of Time</h3>
        <p class="m-0">
          The work recreates a fragile moment with magical colors, where light and dreaminess
          blend, evoking deep emotions in the viewer.
        </p>
      </div>

    </div>
  </section>

  <!-- Section 5 -->
  <section class="section5 container mt-5">
    <div class="row w-100">
      <div class="col-lg-6 col-md-12 col-sm-12 d-flex flex-column justify-content-center">
        <div class="">
          <h3 class="fw-bold text-success">Where Talents Meet</h3>
          <p class="">
            Connecting creative masters and promising young artists, bringing a colorful art space
            for collectors to explore.
          </p>
          <button class="btn btn-success w-50 mt-3">See details</button>
        </div>

      </div>
      <div class="col-lg-6 col-md-12 col-sm-12 mt-lg-0 mt-sm-4">
        <div class="row">
          <div class="col-lg-4 col-md-4 col-sm-4">
            <div class="card">
              <div class="card-body d-flex flex-column align-items-center p-1 gap-2">
                <img src="/src/assets/img/3.png" class="w-100" alt="..." />
                <p class="card-text">Jinnn</p>
              </div>
            </div>
          </div>
          <div class="col-lg-4 col-md-4 col-sm-4">
            <div class="card">
              <div class="card-body d-flex flex-column align-items-center p-1 gap-2">
                <img src="/src/assets/img/3.png" class="w-100" alt="..." />
                <p class="card-text">Jinnn</p>
              </div>
            </div>
          </div>
          <div class="col-lg-4 col-md-4 col-sm-4">
            <div class="card">
              <div class="card-body d-flex flex-column align-items-center p-1 gap-2">
                <img src="/src/assets/img/3.png" class="w-100" alt="..." />
                <p class="card-text">Jinnn</p>
              </div>
            </div>
          </div>


        </div>




      </div>
    </div>
  </section>
</template>


<script>
export default {
  name: "HomeIndex",
};
</script>

<script setup></script>



<style scoped>
.w-400 {
  width: 400px;
}

.h-350 {
  height: 350px;
}

.picture {
  display: flex;
  gap: 20px;
  margin-top: 30px;
  justify-content: flex-end;
  align-items: center;
}





/* .picture {
  gap: 20px;
  margin-top: 30px;
} */

/* .text-success {
  color: #044a42;
} */

.section2 {

  background-color: #044a42;

  box-shadow: inset 0px 10px 4px rgba(0, 0, 0, 0.25),
    inset 0px -10px 4px rgba(0, 0, 0, 0.25);
}



.w-100px {
  width: 100px;
}

.h-100px {
  height: 100px;
}




/*  */
.section4 {

  box-shadow: inset 0px 10px 4px rgba(0, 0, 0, 0.25), inset 0px -10px 4px rgba(0, 0, 0, 0.25);
}

/*  */
.section4 img {
  position: absolute;
  width: 200%;
  top: -80%;
  right: -15%;
}

/* Ảnh nền ban đầu */
.custom-abs-intro {
  top: -110px;
  left: 187px;
  width: 1000px;
  height: 1000px;
  transform: translateX(-39%);
}

.w-180 {
  width: 180px;
}
</style>
